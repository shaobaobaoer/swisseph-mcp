...
func TestHandleCalcFixedStars(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcFixedStars(args)
	if err != nil {
		t.Fatalf("handleCalcFixedStars: %v", err)
	}
	m := result.(map[string]interface{})
	if m["conjunctions"] == nil {
		t.Error("missing conjunctions field")
	}
}

func TestHandleCalcMidpoints(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcMidpoints(args)
	if err != nil {
		t.Fatalf("handleCalcMidpoints: %v", err)
	}
	if result == nil {
		t.Fatal("result is nil")
	}
...
func TestHandleCalcHarmonicChart(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	if err != nil {
		t.Fatalf("handleCalcHarmonicChart: %v", err)
	}
	if result == nil {
		t.Fatal("result is nil")
	}
...
func TestHandleCalcPlanetaryHours(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcPlanetaryHours(args)
	if err != nil {
		t.Fatalf("handleCalcPlanetaryHours: %v", err)
	}
	if result == nil {
		t.Fatal("result is nil")
	}
...
func TestHandleCalcProfection(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcProfection(args)
	if err != nil {
		t.Fatalf("handleCalcProfection: %v", err)
	}
	m := result.(map[string]interface{})
	if m["annual"] == nil {
		t.Error("missing annual field")
	}
	if m["monthly"] == nil {
		t.Error("missing monthly field (include_monthly=true)")
	}
}

func TestHandleCalcProfection_Timeline(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcProfection(args)
	if err != nil {
		t.Fatalf("handleCalcProfection timeline: %v", err)
	}
	m := result.(map[string]interface{})
	if m["timeline"] == nil {
		t.Error("missing timeline field")
	}
}

func TestHandleCalcAntiscia(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcAntiscia(args)
	if err != nil {
		t.Fatalf("handleCalcAntiscia: %v", err)
	}
	m := result.(map[string]interface{})
	if m["antiscia_points"] == nil {
		t.Error("missing antiscia_points field")
	}
}

func TestHandleCalcLots(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	if err != nil {
		t.Fatalf("handleCalcLots: %v", err)
	}
	m := result.(map[string]interface{})
	if m["lots"] == nil {
		t.Error("missing lots field")
	}
}

func TestHandleCalcBounds(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcBounds(args)
	if err != nil {
		t.Fatalf("handleCalcBounds: %v", err)
	}
	m := result.(map[string]interface{})
	if m["faces"] == nil {
		t.Error("missing faces field")
	}
}
...
func TestHandleCalcFixedStars_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcFixedStars(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}

func TestHandleCalcMidpoints_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcMidpoints(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}

func TestHandleCalcHarmonicChart_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcHarmonicChart(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}

func TestHandleCalcPlanetaryHours_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcPlanetaryHours(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}

func TestHandleCalcProfection_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcProfection(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}

func TestHandleCalcAntiscia_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcAntiscia(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}

func TestHandleCalcLots_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcLots(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}

func TestHandleCalcBounds_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcBounds(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}
...
func TestHandleToolsCall_NewTools(t *testing.T) {
	...
	tools := []struct {
		...
		{"calc_bounds", `{"latitude":51.5,"longitude":-0.1,"jd_ut":2451545.0}`},
		{"calc_lunar_phase", `{"jd_ut":2451545.0}`},
		{"calc_antiscia", `{"latitude":51.5,"longitude":-0.1,"jd_ut":2451545.0}`},
		{"calc_lots", `{"latitude":51.5,"longitude":-0.1,"jd_ut":2451545.0}`},
		{"calc_profection", `{"latitude":51.5,"longitude":-0.1,"natal_jd_ut":2451545.0,"age":30}`},
		{"calc_fixed_stars", `{"latitude":51.5,"longitude":-0.1,"jd_ut":2451545.0}`},
		{"calc_midpoints", `{"latitude":51.5,"longitude":-0.1,"jd_ut":2451545.0}`},
		{"calc_harmonic_chart", `{"latitude":51.5,"longitude":-0.1,"jd_ut":2451545.0,"harmonic":5}`},
		{"calc_planetary_hours", `{"latitude":51.5,"longitude":-0.1,"jd_ut":2451545.0}`},
...
func TestHandleCalcDispositors(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcDispositors(args)
	if err != nil {
		t.Fatalf("handleCalcDispositors: %v", err)
	}
	if result == nil {
		t.Fatal("result is nil")
	}
...
func TestHandleCalcDispositors_Traditional(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcDispositors(args)
	if err != nil {
		t.Fatalf("handleCalcDispositors traditional: %v", err)
	}
	if result == nil {
		t.Fatal("result is nil")
	}
...
func TestHandleCalcNatalReport(t *testing.T) {
	s := newTestServer()
	args := json.RawMessage(`{
		"latitude": 51.5074,
		"longitude": -0.1278,
	...
	result, err := s.handleCalcNatalReport(args)
	if err != nil {
		t.Fatalf("handleCalcNatalReport: %v", err)
	}
	if result == nil {
		t.Fatal("result is nil")
	}
...
func TestHandleCalcDispositors_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcDispositors(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}

func TestHandleCalcNatalReport_InvalidJSON(t *testing.T) {
	s := newTestServer()
	_, err := s.handleCalcNatalReport(json.RawMessage(`{invalid`))
	if err == nil {
		t.Error("expected error")
	}
}
...
func TestHandleCalcChartWheel(t *testing.T) {
	...
	result, err := s.handleCalcChartWheel(args)
	if err != nil {
		t.Fatalf("handleCalcChartWheel: %v", err)
	}
	m := result.(map[string]interface{})
	if m["wheel"] == nil {
		t.Error("missing wheel field")
	}
	if m["signs"] == nil {
		t.Error("missing signs field")
	}
}
